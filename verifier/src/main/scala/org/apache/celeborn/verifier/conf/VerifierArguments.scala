...
abstract class VerifierArguments(val args: Array[String], val conf: VerifierConf) {
  ...
  loadDefaultVerifierConf(conf, propertiesFile)
  Utils.loadDefaultCelebornProperties(conf.celebornConf)

  def loadDefaultVerifierConf(conf: VerifierConf, filePath: String = null): Unit = {
    val path = Option(filePath).getOrElse(getDefaultVerifierPropFile())
  ...
  def getDefaultVerifierPropFile(env: Map[String, String] = sys.env): String = {
    env.get("CELEBORN_CONF_DIR")
      .orElse(env.get("CELEBORN_HOME").map { t => s"$t${File.separator}conf" })
      .map { t => new File(s"$t${File.separator}verifier.conf") }
      .filter(_.isFile)
